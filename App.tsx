...
import { LanguageSelectorIntro } from './components/LanguageSelectorIntro';
    ...
    <div className="bg-stone-50 text-stone-900 selection:bg-brand selection:text-white cursor-none relative">
          ...
          <motion.div style={{ y: heroTextY }}>
            <Reveal width="100%">
              <h1 className="text-[18vw] md:text-[12vw] leading-[0.8] font-serif font-medium tracking-tighter text-white mix-blend-overlay">
                ...
                <span className="italic font-light">{t.hero.headline_2}</span>
              </h1>
            </Reveal>
            ...
            <p className="mt-8 text-white/90 max-w-lg mx-auto font-light text-lg tracking-wide drop-shadow-md">{t.hero.subheadline}</p>
          </motion.div>
        </div>
            ...
            <div className="space-y-8">
              ...
              <Reveal>
                <h2 className="text-6xl md:text-7xl font-serif text-stone-900 leading-none">
                  {t.meet_jenny.title_1} <br />
                  <span className="italic text-brand text-8xl">{t.meet_jenny.title_2}</span>
                </h2>
              </Reveal>
              ...
              <Reveal>
                <div className="pt-8">
                  ...
                  <p className="text-xs tracking-widest text-stone-400 mt-2 uppercase">{t.meet_jenny.role}</p>
                </div>
              </Reveal>
            </div>
          </div>
      ...
      <section className="relative py-40 bg-stone-900 text-stone-50 overflow-hidden min-h-screen flex items-center group/spotlight">
        ...
        <div className="relative z-10 max-w-[1600px] mx-auto px-6 md:px-12 w-full">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            <div>
              <SectionHeader title={t.nightmares.header_title} subtitle={t.nightmares.header_subtitle} light />
            ...
            <div className="space-y-4">
              {NIGHTMARES.map((nightmare, i) => (
                ...
                >
                  ...
                  >
                    <p className="text-stone-400 font-light text-lg pl-10">{t.nightmares.items[i].description}</p>
                  </motion.div>
                </div>
              ))}
            </div>
      ...
      <section className="py-20 bg-stone-50 border-b border-stone-200">
        <div className="max-w-[1200px] mx-auto px-6">
          <Reveal>
            <h2 className="text-4xl md:text-5xl font-serif text-stone-900 mb-12 text-center">{t.comparison.title}</h2>
          </Reveal>
          ...
          <div className="grid md:grid-cols-3 gap-0 border border-stone-200 shadow-xl">
            ...
            <div className="hidden md:block bg-stone-100 p-8 border-r border-stone-200">
              <h4 className="font-bold tracking-widest uppercase mb-8">{t.comparison.header_feature}</h4>
              {t.comparison.features.map((c, i) => (
            ...
            <div className="bg-stone-50 p-8 border-r border-stone-200 relative overflow-hidden group">
              ...
              <h4 className="font-bold tracking-widest uppercase mb-8 text-stone-500">{t.comparison.header_them}</h4>
              {t.comparison.features.map((c, i) => (
                <div key={i} className="h-20 flex items-center border-b border-stone-200 last:border-0">
            ...
            <div className="bg-white p-8 relative overflow-hidden">
              ...
              <h4 className="font-bold tracking-widest uppercase mb-8 text-stone-900">{t.comparison.header_us}</h4>
              {t.comparison.features.map((c, i) => (
      ...
      <section className="py-20 bg-stone-50">
        <div className="max-w-[1600px] mx-auto px-6 mb-12 text-center">
          <Reveal>
            <h2 className="text-4xl md:text-6xl font-serif text-stone-900 mb-6">{t.proof.title}</h2>
            <p className="text-stone-500 max-w-2xl mx-auto">{t.proof.subtitle}</p>
          </Reveal>
        </div>
        <div className="h-[70vh] w-full max-w-[1600px] mx-auto rounded-xl overflow-hidden shadow-2xl relative">
          <BeforeAfter
            ...
            beforeLabel={t.proof.before_label}
            afterLabel={t.proof.after_label}
          />
        </div>
      </section>
      ...
      <section ref={galleryRef} className="py-32 bg-stone-50">
        <div className="max-w-[1600px] mx-auto px-6">
          <div className="mb-16 flex flex-col md:flex-row justify-between items-end gap-6 text-center md:text-left">
            <Reveal>
              <h2 className="text-4xl md:text-6xl font-serif text-stone-900">{t.gallery.title}</h2>
              <p className="text-stone-500 mt-4 max-w-xl text-lg">{t.gallery.subtitle}</p>
            </Reveal>
            <Reveal delay={0.2}>
              ...
              <Reveal key={i} delay={i * 0.1}>
                ...
                >
                    ...
                    {img.embedUrl && (
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="bg-white/20 backdrop-blur-sm border border-white/50 text-white px-6 py-3 rounded-full flex items-center gap-3 transition-transform duration-300 group-hover:scale-110 group-hover:bg-white/30 shadow-2xl">
                          ...
                          <span className="font-bold tracking-widest text-xs uppercase text-shadow-sm">{t.gallery.click_tour}</span>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="flex justify-between items-baseline border-b border-stone-200 pb-4 group-hover:border-stone-900 transition-colors duration-500">
                    <h3 className="text-2xl font-serif text-stone-900 group-hover:text-brand transition-colors">{t.gallery.labels[i]}</h3>
              ...
              <div className="order-2 lg:order-1">
                <Reveal>
                  ...
                  <div className="relative z-10">
                    <h3 className="text-5xl md:text-7xl font-serif mb-8">{t.process.steps[i].title}</h3>
        ...
        <div className="max-w-[1600px] mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="order-2 lg:order-1">
              <Reveal>
                <div className="mb-8">
                  <span className="text-xs font-bold tracking-[0.2em] uppercase text-brand bg-brand/20 px-4 py-2 rounded-full">{t.case_study.badge}</span>
                </div>
                <h2 className="text-5xl md:text-7xl font-serif text-stone-900 mb-6 leading-none">
                  {t.case_study.title.split('\n').map((line, i) => (
                    <span key={i} className="block">{line}</span>
                  ))}
                </h2>
                <p className="text-xl font-medium text-stone-900 mb-4">{t.case_study.location}</p>
                <p className="text-stone-600 mb-12 text-lg leading-relaxed">{t.case_study.desc}</p>

                <div className="space-y-8">
                  {t.case_study.points.map((point, i) => (
            ...
            <div className="order-1 lg:order-2 h-[60vh] relative interactive group overflow-hidden">
              ...
              <div className="absolute bottom-8 right-8 bg-white/90 backdrop-blur p-6 max-w-xs z-20 hidden md:block">
                <p className="font-serif text-2xl text-stone-900 italic">{t.case_study.quote}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <Reveal key={i} delay={i * 0.1}>
                <div className={`p-10 border transition-all duration-300 relative group h-full flex flex-col justify-between interactive ${pkg.highlight ? 'bg-stone-900 text-white border-brand shadow-2xl scale-105 z-10' : 'bg-white text-stone-900 border-stone-200 hover:border-brand'}`}>
                  <div>
                    ...
                    <h3 className="text-3xl font-serif mb-4">{t.packages.items[i].name}</h3>
                    <p className={`text-sm mb-8 leading-relaxed ${pkg.highlight ? 'text-stone-400' : 'text-stone-500'}`}>{t.packages.items[i].description}</p>
                    <ul className="space-y-4 mb-12">
                      {t.packages.items[i].features.map((feature, idx) => (
                    ...
                    <div className="mt-auto pt-8 border-t border-stone-200">
                      <p className={`text-4xl font-serif font-bold mb-6 ${pkg.highlight ? 'text-white' : 'text-stone-900'}`}>{t.packages.items[i].price}</p>
                      <Button variant={pkg.highlight ? 'primary' : 'secondary'} fullWidth onClick={() => handleOpenCheckout(pkg)}>
                        {t.packages.items[i].button_text}
                      </Button>
                    </div>
                  </div>
                </div>
      ...
      <section className="py-40 bg-white">
        <div className="max-w-[1600px] mx-auto px-6 grid md:grid-cols-2 gap-24">
          <div>
            <SectionHeader title={t.deliverables.title} subtitle={t.deliverables.subtitle} />
            <div className="h-[600px] w-full rounded-xl overflow-hidden shadow-lg relative interactive group mt-12">
              <img
                src="/deliverables-showcase.webp"
                alt="Design Deliverables Visualization"
                className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
              />
      ...
      <section className="bg-stone-900 text-stone-50 py-40 relative group/spotlight">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-20">
          <h2 className="text-4xl font-serif text-center mb-16">{t.faq.title}</h2>
      ...
      {!hasSelectedLanguage && (
        <LanguageSelectorIntro onSelect={(selectedLang) => {
